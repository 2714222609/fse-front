<template>
  <div>
    <div v-show="select_main || smShow" class="bg-select"></div>
    <!-- 上方导航栏 -->
    <top-bar></top-bar>
    <select-model
      v-show="smShow"
      @closeClick="toggleShow(false)"
      class="select-model"
    ></select-model>
    <!-- 查询的容器 -->
    <div class="banner">
      <div class="site_title">GMFDP</div>
      <div class="small_title">Gut Microbiota Federated Data Platform</div>
      <!-- 查询框 -->
      <div class="serch-bar">
        ...
        <input type="text" class="ipt" placeholder="Input Datalog" v-model="query" />
        <div class="serch">
          <img class="onSearch" src="../assets/lujing1.png" @click="onSearch()" />
        </div>
      </div>
    </div>
    <!-- 选项弹出层 -->
    <div v-show="select_main" class="select-content">
      ...
      <img @click="openselect($event)" src="../assets/cancel.png" class="cancel-icon" />
      <div class="select-item-main">
        ...
        <div class="select-type">
          <select required="required" v-model="selected">
            <option value="" disabled="disabled" selected="selected">operation</option>
